...
pub fn rewrite_small_step() -> Vec<Rewrite<Lambda>> {
    let searcher = Box::new(|eg: &EGraph<Lambda>| {
        ...
        for cand in crate::lambda::big_step::candidates(eg) {
            ...
            for b_node in eg.enodes_applied(&b) {
                future_steps.push((app_id.clone(), x_root, t.clone(), b_node.clone()));
    ...
    let applier = Box::new(
        |future_steps: Vec<(AppliedId, Slot, AppliedId, Lambda)>, eg: &mut EGraph<Lambda>| {
            let mut future_unions = Vec::new();
            for (app_id, x_root, t, b_node) in future_steps {
                let new = step(x_root, t.clone(), &b_node, eg);
                future_unions.push((new, app_id.clone()));
            }
