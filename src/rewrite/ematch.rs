...
pub fn ematch_all<L: Language, N: Analysis<L>>(
    ...
    debug!("ematch_all pattern = {:?}", pattern);
    let mut out = Vec::new();
...
fn ematch_impl<L: Language, N: Analysis<L>>(
    ...
    // debug!("==== ematch_impl input ====");
    // debug!("pattern = {pattern:?}");
    // debug!("st = {st:?}, i = {i:?}");
    // debug!("i eclass = {:#?}", eg.classes[&i.id]);
    match &pattern {
        Pattern::PVar(v) => {
            let mut st = st;
...
fn ematch_node<L: Language, N: Analysis<L>>(
    ...
    'nodeloop: for enode_shape in eg.get_group_compatible_weak_variants(&nn) {
        ...
        for (sub_id, sub_pat) in enode_shape
