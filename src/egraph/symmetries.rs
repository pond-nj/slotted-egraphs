...
impl<L: Language, N: Analysis<L>> EGraph<L, N> {
    #[allow(unused)]
    // TODO: find a heuristic threshold for number of permutations to call SAT
    // fn symmetriesBySat1(self: &mut EGraph<L, N>, src_id: Id) -> (Vec<SlotMap>, Id) {
    ...
    fn getTotalPerms(self: &mut EGraph<L, N>, src_id: Id) -> u128 {
        let pc1 = self.pc_from_src_id(src_id);
        let appIds = pc1.node.elem.applied_id_occurrences();
    ...
    fn symmetriesBySat(self: &mut EGraph<L, N>, src_id: Id) -> (Vec<SlotMap>, Id) {
        ...
        for (_, appId) in appIds.iter().enumerate() {
            for (_, (_, s)) in appId.m.iter().enumerate() {
                if !slotsIdx.contains_key(&s) {
                    slotsIdx.insert(s, slotsIdx.len());
                }
            }
        ...
        // 2) permSlots must be bijection. E.g. if x is permuted to y, then others cannot permute to y
        for (_, i) in slotsIdx.iter() {
            for (y, j) in slotsIdx.iter() {
                trace!("if permSlots[{i}][{y}] then");
                for (z, k) in slotsIdx.iter() {
