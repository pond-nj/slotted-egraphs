...
impl<L: Language> Step<L> {
    fn to_string(&self) -> String {
        if let Some((next, subpos)) = self.rw_pos.split_first() {
            ...
            let SyntaxElem::String(op) = list.remove(0) else {
                unreachable!()
            };
...
impl ProvenEqRaw {
    ...
    fn to_steps<L: Language, N: Analysis<L>>(
        ...
        match eq.proof() {
            ...
            Proof::Symmetry(SymmetryProof(e)) => Self::to_steps(graph, e, pos, !symm, ctx),
...
impl ProvenEqRaw {
    ...
    fn side<L: Language, N: Analysis<L>>(
        &self,
        src: bool,
        symm: bool,
        graph: &EGraph<L, N>,
...
impl<L: Language> FlatteningContext<L> {
    ...
    fn update_slot_map_core(map: &mut BTreeMap<Slot, Slot>, head: &RecExpr<L>, src: &RecExpr<L>) {
        ...
        let [SyntaxElem::String(src_op), src_children @ ..] = &*src.node.to_syntax() else {
            unreachable!()
        };
        ...
        for idx in 0..head_children.len() {
            match (&head_children[idx], &src_children[idx]) {
                (SyntaxElem::AppliedId(_), SyntaxElem::AppliedId(_)) => {
                    Self::update_slot_map_core(
                        map,
                        &head.children[child_idx],
                        &src.children[child_idx],
                    );
                    child_idx += 1;
                }
